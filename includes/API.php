...
class API extends ApiBase {
	...
	private function executeListAll() {
		...
		if ( !$priviledged ) {
			...
			do {
			} while ( Helper::batchFetchParent( $query->posts ) );
			$visible = [];
			foreach ( $posts as $post ) {
				if ( $post->isVisible() ) {
					$visible[] = $post;
	...
	public function execute() {
		...
		if ( $wgFlowThreadEnforcePost && $writeMode && $this->getRequest()->getMethod() !== 'POST' ) {
			$this->dieWithError(
				[
